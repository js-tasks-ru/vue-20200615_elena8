<template>
  <div :class="{dropdown: true, show: this.open }">
    <button type="button" :class="{button:true, dropdown__toggle:true, dropdown__toggle_icon:hasIcons}" @click="toggle">
      <app-icon v-if="selected.icon" :icon="selected.icon" />
      {{value ? title + ' - ' + selected.text : title}}
    </button>

    <div :class="{dropdown__menu: true, show: this.open }">
      <button 
        :class="{dropdown__item: true, dropdown__item_icon:option.icon}" 
        type="button" 
...
export default {
  ...
  computed: {
    selected() {
      ...
      // console.log(JSON.parse(JSON.stringify(this.options.find((el) => el.value === this.value))));
